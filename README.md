...
| ⌨️ **表情短码** | 输入 `:smile:` `:joy:` `:heart:` 等短码自动替换为 Emoji |
| ⌨️ **/emoji 命令** | `:/emoji smile` 直接替换为 😀，`/emoji` 单独输入弹出面板 |
...
| ❤️ **心跳检测** | 客户端每 30 秒发 PING，服务端 60 秒超时自动清理僵尸连接 |
...
| 维度 | 选型 | 理由 |
|------|------|------|
| 🖥️ **服务端语言** | C++17 | 系统编程、Socket 接口原生支持 |
...
| `/w 用户名 消息` | 发送私聊 |
| `/emoji` | 弹出表情选择面板 |
| `/emoji 名称` | 按名称插入表情（如 `/emoji smile` → 😀） |
...
- [x] epoll 边缘触发高并发模型
- [x] 自定义二进制通信协议
- [x] 😄 表情面板（六大分类，200+ Emoji）
- [x] ⌨️ Emoji 短码自动替换（`:smile:` → 😀）
- [x] 🆔 @ 提及用户（弹出列表 + 自动补全 + 蓝色高亮）
- [x] 🔔 被 @ 时窗口闪烁提醒
- [x] 📝 聊天记录自动保存到本地文件
- [x] ❤️ 客户端定时心跳（30s） + 服务端超时检测（60s）
...
如果你要用这个项目写课设报告，推荐以下大纲：

1. **需求分析** — 聊天室的功能需求与非功能需求
2. **概要设计** — 整体架构图、C/S 交互流程
3. **详细设计**
   - 通信协议设计（为什么选二进制长度前缀）
   - epoll 模型分析（对比 select/poll，ET vs LT）
   - 客户端状态机与线程模型
4. **核心代码讲解** — 拆解 3-5 个关键函数/类
5. **运行截图** — 登录 / 群聊 / 私聊 / 用户列表 / 断开重连
6. **总结与展望** — 踩过的坑、后续改进
